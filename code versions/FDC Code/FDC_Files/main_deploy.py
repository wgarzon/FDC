...
git = "https://github.com/wgarzon/FDC.git"
init_cmd_all =  """mkdir -p ~/Remote_FDC_RF/"""
init_cmd_site = """mkdir -p ~/Remote_FDC_RF/data/"""
init_sites = """"scp -o BatchMode=yes -o PasswordAuthentication=no -o StrictHostKeyChecking=no rm -rf ~/Remote_FDC_RF/"""
...
def sub_deploy(nodes_number):
    [(jobid,site)] = oarsub([(OarSubmission(resources="{cluster='gros'}/nodes="+str(nodes_number), job_type='deploy'), 'nancy')])
...
def installLibraries(nodes):
    ...
    exec_cmd_as_root("pip3 install -U scikit-learn pandas matplotlib imblearn && pip3 install pyzmq && pip3 install gdown", nodes)
...
def main():
    ...
    Get(mainCsite, ["~/Remote_FDC_RF/shareModels/outcomes/"], "/home/wgarzon/getResults/", connection_params = {'user': 'root'}).run()
    Get(mainCsite, ["~/Remote_FDC_RF/shareModels/Ensembled/"], "/home/wgarzon/getResults/", connection_params = {'user': 'root'}).run()
