...
def loadLocalData(pathOut, site):
    Xtest = pd.read_csv(pathOut+"Site"+str(site)+"_xtest.csv")
    Xtrain = pd.read_csv(pathOut+"Site"+str(site)+"_xtrain.csv")
    ytest = pd.read_csv(pathOut+"Site"+str(site)+"_ytest.csv")
    ytrain= pd.read_csv(pathOut+"Site"+str(site)+"_ytrain.csv")

    return Xtrain, Xtest, ytrain, ytest

def loadAggData(pathOut):
    Xtest = pd.read_csv(pathOut+"xtest_Agg.csv")
    ytest = pd.read_csv(pathOut+"ytest_Agg.csv")
    #Xtrain = pd.read_csv(pathOut+"xtrain_Agg.csv")
    #ytrain= pd.read_csv(pathOut+"ytrain_Agg.csv")
    #return Xtrain, Xtest, ytrain, ytest
    return None, Xtest, None, ytest
...
def saveMdlsByAcc(mdlsByAcc, ClusterFeats, site, path,pathShare):
    #dump(mdlsByAcc[0], path + site + "Model_Bad.Accmodel")
    dump(mdlsByAcc[1], path + site + "Model_Best.Accmodel")
    dump(mdlsByAcc[1], pathShare + site + "Model_Best.Accmodel")

    print("No of Best models:",len(mdlsByAcc[1]), ". No of Bad models:", len(mdlsByAcc[0]))
    print("Share Models stored on disk!!!")


def aggDataSets(path):
    #files = {"xtest":None,"xtrain":None,"ytest":None,"ytrain":None}
    files = {"xtest":None,"ytest":None}
    for typ in files:
        dfs = []
        allffs = [path + i for i in os.listdir(path) if i.endswith(typ+".csv")]
        for ff in allffs: dfs.append(pd.read_csv(ff))

        res = pd.concat(dfs)
        files[typ]= res
        res.to_csv(path +"Ensembled/"+ typ + "_Agg.csv", index=False)

    print("All files generated to be shared with all sites")
    #return files["xtrain"]
