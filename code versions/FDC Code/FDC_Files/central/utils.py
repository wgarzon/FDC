...
def preprocess_ds(dsName, norm=True, genFiles=False):
...
def save_VI_file(model, df, site , outDir,idd=0):
    ...
    head = False
    if not os.path.exists(outDir+'outcomes/VIM.csv'): head = True
    res.to_csv(outDir+'outcomes/VIM_'+site+'.csv', mode='a',  index=False, header=head)
    plot_VIM(fi[:30],site,outDir)

#Plot features by level importance
def plot_VIM(df,site,outDir):
    ...
    plt.savefig(outDir + "figs/VIM_" + site+xx+'.png')
...
def plot_OOB(d,y_label,site, outDir):
    ...
    xx = datetime.now().strftime("_%d%m%Y_%H%M")
    fig.savefig(outDir + 'figs/OOB_'+site+xx+'.png')
