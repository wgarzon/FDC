...
ModelParam = {"Random Forest": {"n_estimators": 100, "max_features": "log2", "oob_score": True, "n_jobs": -1},
...
def splitMdlsByAcc(clusters, OriModel, data,modelName):
    ...
    for id_CL in clusters:
        ...
        feats = []
        for id_DT in range(len(valClusters)):
            nModel.estimators_[id_DT].tree_ = OriModel[id_DT + 1][0]
            feats.append(OriModel[id_DT + 1][1])

        ClusModels[id_CL] = nModel
        FeatModels[id_CL] = feats

    return ClusModels, FeatModels
...
def ScoreModel(site, model, Xtest, ytest):
    try:
        ...
        print(text)
        ff = open('out/Results.txt', 'a')
        ff.write(site + ","+ text + ","+ str(datetime.now().strftime("%d%m%Y_%H%M")) + "\n")
